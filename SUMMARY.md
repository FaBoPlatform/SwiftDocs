# Summary
* [はじめに](README.md)
* [UIKit](uikit/README.md)
	* [001 UILabelで文字を表示](uikit/001_uilabel.md)
	* [002 UIButtonでボタンを表示](uikit/002_uibutton.md)
	* [003 UIButtonでいろいろなボタンを表示](uikit/003_uibutton.md)
	* [004 UITextFieldで文字を入力](uikit/004_uitextfield.md)
	* [005 UIImageViewで画像を表示](uikit/005_uiimageview.md)
	* [006 UITableViewでテーブルを表示](uikit/006_uitableview.md)
	* [007 UIFontで文字を表示](uikit/007_uifont.md)
	* [008 UserNotificationの表示](uikit/008_uinotification.md)
	* [009 UIWebViewでWebページを表示](uikit/009_uiwebview.md)
	* [010 UIAlertControllerでアラートを表示](uikit/010_uialertcontroller.md)
	* [011 UIPickerViewで値の選択](uikit/011_uipickerview.md)
	* [012 UINavigationControllerの表示](uikit/012_uinavigationcontroller.md)
	* [013 UITabBarControllerでタブの表示](uikit/013_uitabbarcontroller.md)
	* [014 UIImageViewで画像の表示](uikit/014_uiimageview.md)
	* [015 UIViewControllerで画面遷移](uikit/015_uiviewcontroller.md)
	* [016 UIScrollViewの表示](uikit/016_uiscrollview.md)
	* [017 UIBarButtonItemの設置](uikit/017_uibarbuttonitem.md)
	* [018 UIPageControlの表示](uikit/018_uipagecontrol.md)
	* [019 セクション分けしたUITableViewを作る](uikit/019_sectionuitableview.md)
	* [020 UITextViewで複数行のテキストの表示](uikit/020_uitextview.md)
	* [021 UISliderの表示](uikit/021_uislider.md)
	* [022 UISwitchの表示](uikit/022_uiswitch.md)
	* [023 UIDatePickerの表示](uikit/023_uidatepicker.md)
	* [024 UIActivityIndicatorの表示](uikit/024_uiactivityindicator.md)
	* [025 UISearchBarの表示](uikit/025_uisearchbar.md)
	* [026 UIWindowの表示](uikit/026_uiwindow.md)
	* [027 UIToolBarの表示](uikit/027_uitoolbar.md)
	* [028 UIProgressViewで進捗の具合の確認](uikit/028_uiprogressview.md)
	* [029 UISegmentedControlの表示](uikit/029_uisegmentedcontrol.md)
	* [030 UIStepperの表示](uikit/030_uistepper.md)
	* [031 UINavigationItemでUINavigationBarを設定](uikit/031_uinavigationitem.md)
	* [032 UIAlertControllerでActionSheet表示](uikit/032_uialertC¥controller.md)
	* [033 UIMenuControllerにMenuItemを追加](uikit/033_uimenucontroller.md)
	* [034 UIViewにUIMotionEffectを追加](uikit/034_uimotioneffect.md)
	* [035 UIViewにBlurエフェクトを適用させる](uikit/035_blureffect.md)
	* [036 UIViewにトランジションをつける](uikit/036_uiviewtransition.md)
	* [037 タッチイベントの有効/無効を切り替える](uikit/037_touchevent.md)
	* [038 UITextFieldに文字数制限を設ける](uikit/038_uitextfield.md)
	* [039 UITextFieldの入力をパスワード形式にする](uikit/039_uitextfield.md)
	* [040 GestureRecognizerのイベント取得](uikit/040_gesturerecognizer.md)
	* [041 UIAlertControllerをパスワード形式にする](uikit/041_uiapplicationpass.md)
	* [042 UIAlertControllerで文字数制限を設ける](uikit/042_uialertcontroller.md)
	* [043 プレビュー画面にエフェクト処理](uikit/043_preview.md)
	* [044 セカンドScreenへの描画](uikit/044_scoundscreendraw.md)
	* [045 TableViewのボタンの拡張](uikit/045_uitableviewbutton.md)
	* [046 UITableViewをSlideさせる](uikit/046_uitableviewslide.md)
	* [047 周期の違うUIScrollViewを同期させる](uikit/047_uiscrollview.md)
	* [048 プルンプルンするButtonの作成](uikit/048_effectbutton.md)
	* [049 カスタマイズButton](uikit/049_custombutton.md)
	* [050 UIパーツの削除](uikit/050_deleteuiparts.md)
	* [051 UITableViewに追加・削除機能を追加](uikit/051_uitableview.md)
	* [052 UIBezierPathで図形を描画](uikit/052_uibezierpath.md)
	* [053 UICollectionViewを使う](uikit/053_uicollectionview.md)
	* [054 UICollectionViewのCellをカスタマイズする](uikit/054_customuicollectionview.md)
	* [055 UICollectionViewをセクション毎に分ける](uikit/055_sectionuicollectionview.md)
	* [056 UIImagePickerControllerでライブラリから画像を選択](uikit/056_uiimagepickercontroller.md)
	* [057 UIImageで画像の透過](uikit/057_uiimage.md)
	* [058 UIImageをぼかす](uikit/058_uiimage.md)
	* [059 UIImageをリサイズする](uikit/059_uiimage.md)
	* [060 UIImageを合成する](uikit/060_uiimage.md)
	* [061 UIViewからUIImageを取得する（スクリーンショット）](uikit/061_screenshot.md)
	* [062 UIPopoverControllerを使ってViewをポップさせる(iPad専用)](uikit/062_uipopover.md)
	* [063 UITextCheckerでスペルチェック](uikit/063_uitextchecker.md)
	* [064 UIViewにアタッチメントを適用させる](uikit/064_uiview.md)
	* [065 UIWebViewでファイルの表示](uikit/065_collision.md)
	* [066 UIViewに重力を適用させる](uikit/066_uiview.md)
	* [067 UIViewをスナップさせる](uikit/067_uiview.md)
	* [068 UIViewを動的に動かす](uikit/068_uiview.md)
	* [069 UIViewアニメーションまとめ](uikit/069_animation.md)
	* [070 ドラッグでViewを移動させる](uikit/070_uitouch.md)
	* [071 モーダル表示させる](uikit/071_modal.md)
	* [072 一つのボタンから複数のボタンが飛び出す](uikit/072_custombutton.md)
	* [073 現Viewの上に別Viewを表示する](uikit/073_uiview.md)
	* [074 UIWebViewでPDFを開く](uikit/074_webviewpdf.md)
	* [075 UIWebViewでファイルの表示](uikit/075_webviewfile.md)
* [Social](social/README.md)
	* [001 Twitterに投稿](social/001_twitter.md)
	* [002 Facebookに投稿](social/002_facebook.md)
* [MapKit](mapkit/README.md)
    * [001 地図の表示](mapkit/001_mapview.md)
    * [002 地図の縮尺の変更](mapkit/002_mapview.md)
    * [003 現在位置の地図の表示](mapkit/003_cllocation.md)
    * [004 指定した座標にピンを立てる](mapkit/004_point.md)
    * [005 3D表示](mapkit/005_3d.md)
    * [006 ピンを長押しした地点にドロップさせる](mapkit/006_pin.md)
    * [007 アノテーションに画像を追加](mapkit/007_image.md)
    * [008 アノテーションを任意の画像にする](mapkit/008_image.md)
    * [009 ルート検索をする](mapkit/009_route.md)
    * [010 ローカル検索](mapkit/010_local.md)
    * [011 地図のタイプを指定](mapkit/011_maptype.md)
    * [012 地図上に図形を描く(円)](mapkit/012_map.md)
    * [013 地図上に直線を引く](mapkit/013_line.md)
    * [014 地図のスナップショットを撮る](mapkit/014_snapshot.md)
    * [015 正ジオコーディング](mapkit/015_geo.md)
    * [016 逆ジオコーディング](mapkit/016_geo.md)
* [CoreMotion](coremotion/README.md)
    * [001 加速度センサーの値を取得](coremotion/001_coremotion.md)
    * [002 活動量計の値の取得](coremotion/002_coremotion.md)
    * [003 万歩計の歩数の取得](coremotion/003_coremotion.md)
    * [004 高度の計測](coremotion/004_coremotion.md)
* [AVFoundation](avfoundation/README.md)
    * [001 音源の再生](avfoundation/001_avfoundation.md)
    * [002 カメラの起動と画像の保存](avfoundation/002_avfoundation.md)
    * [003 動画の撮影](avfoundation/003_avfoundation.md)
    * [004 撮影した動画を保存する](avfoundation/004_avfoundation.md)
    * [005 動画から静止画を取り出す](avfoundation/005_avfoundation.md)
    * [006 動画の再生](avfoundation/006_avfoundation.md)
    * [007 MediaPlayerを使った動画の再生](coremotion/007_avfoundation.md)
    * [008 QRCodeリーダー](avfoundation/008_avfoundation.md)
* [Connection](conection/README.md)
	* [001 NSURLConnectionを用いたHTTP通信(非同期)](connection/001_nsurlconnection.md)
	* [002 NSURLConnectionを用いたHTTP通信(同期)](connection/002_nsurlconnection.md)
	* [003 NSURLSessionを用いたHTTP通信(Delegateを用いた処理)](connection/003_nsurlsession.md)
	* [004 NSURLSessionを用いたHTTP通信(Handlerを用いた処理)](connection/004_nsurlsession.md)
	* [005 通信の進捗を知る](connection/005_progress.md)
	* [006 タイムアウトの指定](connection/006_timeout.md)
	* [007 POSTを送信する](connection/007_post.md)
	* [008 ファイルのダウンロード](connection/008_download.md)
	* [009 ファイルのアップロード](connection/009_upload.md)
	* [010 JSONデータのPOST](connection/010_postjson.md)
	* [011 JSONデータのパース](connection/011_persejson.md)
	* [012 データの更新日時を取得](connection/012_lastmodified.md)
* [CoreLocation](corelocation/README.md)
	* [001 現在位置を取得](corelocation/001_gps.md)
	* [002 iBeaconをアドバタイジング](corelocation/002_ibeacon_advertising.md)
	* [003 iBeaconをモニタリング](corelocation/003_ibeacon_monitaring.md)
* [Config](config/README.md)
	* [001 Versionを表示する](config/001_version.md)
	* [002 ProvisioningからDevloperかDistributionかを判定する](config/002_provisioning.md)
* [CoreData](config/README.md)
	* [001 JSONデータをCoreDataに保存](coredata/001_json.md)
* [SpriteKit](spritekit/README.md)
    * [000 GameViewControllerの設定をする](spritekit/000_spritekit.md)
    * [001 画面のサイズを得る](spritekit/001_spritekit.md)
    * [002 図形を描画する](spritekit/002_spritekit.md)
    * [003 Nodeの位置を得る](spritekit/003_spritekit.md)
    * [004 Nodeの描画順を設定する](spritekit/004_spritekit.md)
    * [005 Nodeのサイズを変える](spritekit/005_spritekit.md)
    * [006 Nodeに子を追加する](spritekit/006_spritekit.md)
    * [007 親Nodeから子Nodeを全て削除する](spritekit/007_spritekit.md)
    * [008 親Nodeから特定のNodeを削除する](spritekit/008_spritekit.md)
    * [009 Nodeに名前をつける](spritekit/009_spritekit.md)
    * [010 親Nodeの中の特定のnameのNodeを全て得る](spritekit/010_spritekit.md)
    * [011 Nodeを非表示にする](spritekit/011_spritekit.md)
    * [012 Nodeを特定の方向に向かせ続ける](spritekit/012_spritekit.md)
    * [013 Nodeを特定の座標に向かせ続ける](spritekit/013_spritekit.md)
    * [014 Sceneの背景色を変える](spritekit/014_spritekit.md)
    * [015 タッチイベントを取得する](spritekit/015_spritekit.md)
    * [016 タッチした座標にNodeを移動させる](spritekit/016_spritekit.md)
    * [017  剛体同士の衝突を検知する](spritekit/017_spritekit.md)
    * [018  文字列を描画する](spritekit/018_spritekit.md)
    * [019  ラベルのフォントを変える](spritekit/019_spritekit.md)
    * [020  円を描画する](spritekit/020_spritekit.md)
    * [021  四角形を描画する](spritekit/021_spritekit.md)
    * [022  線を描画する](spritekit/022_spritekit.md)
    * [023  三角形を描画する](spritekit/023_spritekit.md)
    * [024  線の太さを変える](spritekit/024_spritekit.md)
    * [025  線の色を変える](spritekit/025_spritekit.md)
    * [026  SKShapeNodeにテクスチャを貼る](spritekit/026_spritekit.md)
    * [027  画像を描画する](spritekit/027_spritekit.md)
    * [028  Nodeを非表示にする](spritekit/028_spritekit.md)
    * [029  Nodeを表示するアクション](spritekit/029_spritekit.md)
    * [030  NodeをRectに従って動かせるアクションを作る](spritekit/030_spritekit.md)
    * [031  指定した回転値まで回転させるアクションを作る](spritekit/031_spritekit.md)
    * [032  指定したスケール値まで拡大させるアクションを追加](spritekit/032_spritekit.md)
    * [033  指定した座標に移動するアクションを作る](spritekit/033_spritekit.md)
    * [034  相対的に回転させるアクションを作る](spritekit/034_spritekit.md)
    * [035  相対的に拡大させるアクションを作る](spritekit/035_spritekit.md)
    * [036  相対的に移動するアクションを作る](spritekit/036_spritekit.md)
    * [037  Nodeをフェードインさせる](spritekit/037_spritekit.md)
    * [038  Nodeをフェードアウトさせる](spritekit/038_spritekit.md)
    * [039  アクションを無限スープさせる](spritekit/039_spritekit.md)
    * [040  アクションを逆転させる](spritekit/040_spritekit.md)
    * [041  複数のアクションを同時実行させる](spritekit/041_spritekit.md)
    * [042  複数のアクションを逐次実行させる](spritekit/042_spritekit.md)
    * [043  親との継承関係を削除するアクション](spritekit/043_spritekit.md)
    * [044  Nodeを特定の時間待機させる](spritekit/044_spritekit.md)
    * [045  Nodeをランダムな時間待機させる](spritekit/045_spritekit.md)
    * [046  Nodeをパスに従って動かすアクション](spritekit/046_spritekit.md)
* [CoreImage](coreimage/README,md)
    * [001 セピア処理](coreimage/001_coreimage.md)
    * [002 画像のリサイズ](coreimage/002_coreimage.md)
    * [003 画像のクロップ](coreimage/003_coreimage.md)
    * [004 顔認識](coreimage/004_coreimage.md)
    * [005 モザイク処理](coreimage/005_coreimage.md)
    * [006 ぼかし処理](coreimage/006_coreimage.md)
    * [007 画像の色の反転](coreimage/007_coreimage.md)
    * [008 モノクロ処理](coreimage/008_coreimage.md)
    * [009 画像の色変換](coreimage/009_coreimage.md)
    * [010 画像の境界をシャープにする](coreimage/010_coreimage.md)
    * [011 画像の合成](coreimage/011_coreimage.md)
    * [012 コピーマシントランジションフィルタ処理](coreimage/012_coreimage.md)
    * [013 スワイプマシントランジションフィルタ処理](coreimage/013_coreimage.md)
    * [014 バースワイプマシントランジションフィルタ処理](coreimage/014_coreimage.md)
    * [015 フラッシュトランジションフィルタ処理](coreimage/015_coreimage.md)
    * [016 モッドトランジションフィルタ処理](coreimage/016_coreimage.md)
    * [017 ディゾルブトランジションフィルタ処理](coreimage/017_coreimage.md)
    * [018 画像のピンチ処理](coreimage/018_coreimage.md)
    * [019 画像のブルーム処理](coreimage/019_coreimage.md)
* [Utility](utility/README.md)
	* [001 タイマーを作る](utility/001_maketimer.md)
	* [002 タイマーを止める](utility/002_stoptimer.md)
	* [004 バッテリーの残量を取得する](utility/004_battery.md)
	* [005 設定画面を開く](utility/005_settingview.md)
	* [007 URLスキームによるメーラーの呼び出し](utility/007_urlscheme.md)
	* [008 現在時間の取得](utility/008_gettime.md)
	* [009 OS情報の取得](utility/009_getosinfo.md)
	* [010 端末の向きが変わったことを検知する](utility/010_getdeviceorientation.md)
	* [011 画面サイズを取得する](utility/011_getscreensize.md)
	* [012 NSUserDefaultsを用いたデータ保存](utility/012_userdefault.md)
* [UserNotification](usernotification/README.md)
	* [001 通知設定のダイアログ設定を開く](usernotification/001_notification.md)
	* [002 アプリのアイコンにバッジを表示する](usernotification/002_badge.md)
* [PhotoKit](photokit/README.md)
    * [001 ユーザーに許可を得る](photokit/001_photokit.md)
    * [002 アルバムを作る](photokit/002_photokit.md)
    * [003 アルバムを検索する](photokit/003_photokit.md)
    * [004 アルバムを削除する](photokit/004_photokit.md)
    * [005 アルバムにアセットを追加する](photokit/005_photokit.md)
    * [006 アルバムのアセットを削除する](photokit/006_photokit.md)
* [GoogleMaps](googlemap/README.md)
	* [001 Swiftで使用するための設定](googlemap/001_setting.md)
	* [002 GoogleMapを表示する](googlemap/002_googlemap.md)
	* [003 指定した座標を表示](googlemap/003_mappoint.md)
	* [004 マーカーを表示](googlemap/004_setmarker.md)
	* [005 StreetView](googlemap/005_streetview.md)
* [CoreBluetooth](corebluetooth/README.md)
    * [001 Bluetooth機器の検出](corebluetooth/001_corebluetooth.md)
    * [002 Peripheralへの接続とServiceの検索](corebluetooth/002_corebluetooth.md)
    * [003 Serviceの接続とCharacteristicsの検索](corebluetooth/003_corebluetooth.md)
    * [004 Read](corebluetooth/004_corebluetooth.md)
    * [005 Write](corebluetooth/005_corebluetooth.md)
    * [006 操作アプリ](corebluetooth/006_corebluetooth.md)
* [HealthKit](healthkit/README.md)
    * [001 認証する](healthkit/001_healthkit.md)
    * [002 体重データのHealthStoreへの読み書き](healthkit/002_healthkit.md)
    * [003 ユーザー情報の取得](healthkit/003_healthkit.md)
    * [004 Pulse oximeter(IEEE Std 11073-10404 Dev specialization)](healthkit/004_healthkit.md)
    * [005  Basic ECG(IEEE Std 11073-10406 Dev specialization)](healthkit/005_healthkit.md)
    * [006 Blood pressure monitor(IEEE Std 11073-10407 Dev specialization)](healthkit/006_healthkit.md)
    * [007 Thermometer (IEEE Std 11073-10408 Dev specialization)](healthkit/007_healthkit.md)
    * [008 Weighing scale (IEEE Std 11073-10415 Dev specialization)](healthkit/008_healthkit.md)
    * [009 Glucose meter + Revision (IEEE Std 11073-10417 Dev specialization)](healthkit/009_healthkit.md)
    * [010 Body comp (IEEE Std 11073-10420 Dev specialization)](healthkit/010_healthkit.md)
    * [011 睡眠情報の記録](healthkit/011_healthkit.md)
    * [012 栄養素の記録](healthkit/012_healthkit.md)
    * [013 運動量の記録](healthkit/013_healthkit.md)
    * [014 取得データの合計値を得る](healthkit/014_healthkit.md)
    * [015 取得データの平均値を得る](healthkit/015_healthkit.md)
    * [016 取得データの最大値・最小値を得る](healthkit/016_healthkit.md)
* iTunes Store
    * [001 Push Notificationの設定](itunes/pushnotification.md)
    * [002 Amazon SNSの設定](itunes/amazonsns.md)
    * [003 Store upload時のエラー](itunes/error.md)
