...
![Preview healthkit009](./img/healthkit012.png)
...
class ViewController: UIViewController, UITextFieldDelegate{
    ...
    var myReadCholesterolField: UITextField!
    var myWriteCholesterolField: UITextField!
    ...
    override func viewDidLoad() {
        ...
        // 入力フィールドを設置.
        myWriteCholesterolField = UITextField(frame: CGRect(x: 0, y: 0, width: 300, height: 30))
        myWriteCholesterolField.placeholder = "コレステロールの摂取量を入力してください"
        myWriteCholesterolField.delegate = self
        myWriteCholesterolField.borderStyle = UITextBorderStyle.roundedRect
        myWriteCholesterolField.layer.position = CGPoint(x: self.view.bounds.width/2, y: 200);
        self.view.addSubview(myWriteCholesterolField)
        ...
        myWriteButton.setTitle("コレステロールの摂取量の書き込み", for: UIControlState.normal)
        ...
        // 表示フィールドを設置.
        myReadCholesterolField = UITextField(frame: CGRect(x: 0, y: 0, width: 300, height: 30))
        myReadCholesterolField.placeholder = "前回登録のコレステロールの摂取量"
        myReadCholesterolField.isEnabled = false
        myReadCholesterolField.delegate = self
        myReadCholesterolField.borderStyle = UITextBorderStyle.roundedRect
        myReadCholesterolField.layer.position = CGPoint(x:self.view.bounds.width/2,y:350);
        myReadCholesterolField.isEnabled = false
        self.view.addSubview(myReadCholesterolField)
        ...
        myReadButton.setTitle("コレステロールの摂取量の読み込み", for: UIControlState.normal)
    ...
    func onClickMyButton(sender: UIButton){
        if(sender.tag == 1){
            ...
            if let val = Double(myWriteCholesterolField.text!) {
                writeData(cholesterol: val)
            }
        }
    }
    ...
    private func readData() {
        // 取得したいデータのタイプを生成.
        let typeOfCholesterol = HKSampleType.quantityType(forIdentifier: HKQuantityTypeIdentifier.dietaryCholesterol)
        ...
        // queryを生成.
        let mySampleQuery = HKSampleQuery(sampleType: typeOfCholesterol!, predicate: predicate, limit: 1, sortDescriptors: [mySortDescriptor])
        { (sampleQuery, results, error ) -> Void in
            // 一番最近に登録されたデータを取得.
            guard let myRecentSample = results!.first as? HKQuantitySample else {
                print("error")
                self.myReadCholesterolField.text = "Data is not found"
                return
            }
            // 取得したサンプルを単位に合わせる.
            DispatchQueue.main.async {
                self.myReadCholesterolField.text = "\(myRecentSample.quantity)"
            }
        }
        // queryを発行.
    ...
    /*
     データの書き込み.
     */
    private func writeData(cholesterol: Double){
        // 入力された値をmgとして扱う.
        let mg = HKUnit.gramUnit(with: .milli)
        // Cholesterolのタイプ.
        let typeOfCholesterol = HKObjectType.quantityType(forIdentifier: HKQuantityTypeIdentifier.dietaryCholesterol)
        // Cholesterolデータを作成.
        let myCholesterol = HKQuantity(unit: mg, doubleValue: cholesterol)
        // StoreKit保存用データを作成.
        let myCholesterolData = HKQuantitySample(type: typeOfCholesterol!, quantity: myCholesterol, start: Date(), end: Date())
        // HealthStoreにデータを保存.
        myHealthStore.save(myCholesterolData, withCompletion: {success, error in
            if let e = error {
                print("Error: \(e.localizedDescription)")
                return
            }
...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        // 入力フィールドを設置.
        myWriteCholesterolField = UITextField(frame: CGRectMake(0,0,300,30))
        myWriteCholesterolField.placeholder = "コレステロールの摂取量を入力してください"
        myWriteCholesterolField.delegate = self
        myWriteCholesterolField.borderStyle = UITextBorderStyle.RoundedRect
        myWriteCholesterolField.layer.position = CGPoint(x:self.view.bounds.width/2,y:200);
        ...
        myWriteButton.layer.masksToBounds = true
        myWriteButton.setTitle("コレステロールの摂取量の書き込み", forState: UIControlState.Normal)
        ...
        // 表示フィールドを設置.
        myReadCholesterolField = UITextField(frame: CGRectMake(0,0,300,30))
        myReadCholesterolField.placeholder = "前回登録のコレステロールの摂取量"
        myReadCholesterolField.enabled = false
        myReadCholesterolField.delegate = self
        myReadCholesterolField.borderStyle = UITextBorderStyle.RoundedRect
        myReadCholesterolField.layer.position = CGPoint(x:self.view.bounds.width/2,y:350);
        myReadCholesterolField.enabled = false
        ...
        myReadButton.layer.masksToBounds = true
        myReadButton.setTitle("コレステロールの摂取量の読み込み", forState: UIControlState.Normal)
    ...
    private func readData() {
        // 取得したいデータのタイプを生成.
        let typeOfCholesterol = HKSampleType.quantityTypeForIdentifier(HKQuantityTypeIdentifierDietaryCholesterol)
    ...
    private func writeData(cholesterol: Double){
        ...
        // Cholesterolのタイプ.
        let typeOfCholesterol = HKObjectType.quantityTypeForIdentifier(HKQuantityTypeIdentifierDietaryCholesterol)
        // Cholesterolデータを作成.
        let myCholesterol = HKQuantity(unit: mg, doubleValue: cholesterol)
        // StoreKit保存用データを作成.
        let myCholesterolData = HKQuantitySample(type: typeOfCholesterol!, quantity: myCholesterol, startDate: NSDate(), endDate: NSDate())
        
        // HealthStoreにデータを保存.
        myHealthStore.saveObject(myCholesterolData, withCompletion: {success, error in
            if let e = error {
                print("Error: \(e.localizedDescription)")
                return
            }
