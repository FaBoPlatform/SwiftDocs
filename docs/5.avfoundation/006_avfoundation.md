...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        startButton.addTarget(self, action: #selector(onButtonClick(sender:)), for: UIControl.Event.touchUpInside)
...
* `AVLayerVideoGravityResizeAspect` が `AVLayerVideoGravity.resizeAspect` に変更
* `UIControlEvents` が `UIControl.Event` に変更
* `func onButtonClick(sender : UIButton)` と `func onSliderValueChange(sender : UISlider)` に `@objc` を追加
* `(interval, Int32(NSEC_PER_SEC))` が `interval, preferredTimescale: Int32(NSEC_PER_SEC))` に変更
* `UIControlState` が `UIControl.State.normal` に変更
* `CMTimeMakeWithSeconds(0, Int32(NSEC_PER_SEC))` が `CMTimeMakeWithSeconds(0, preferredTimescale: Int32(NSEC_PER_SEC))` に変更
* `CMTimeMakeWithSeconds(Float64(seekBar.value), Int32(NSEC_PER_SEC))` が `CMTimeMakeWithSeconds(Float64(seekBar.value), preferredTimescale: Int32(NSEC_PER_SEC))` に変更
